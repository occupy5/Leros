...
Leros 前端工程 — 基于 pnpm + Turborepo 的 monorepo 架构，包含 Web 应用和 Electron 桌面应用，共享 UI 组件库与状态管理包。
...
Web 和 Desktop 应用共享同一套 UI 组件库（`@leros/ui`）和状态管理（`@leros/store`），但使用不同的框架和路由：
